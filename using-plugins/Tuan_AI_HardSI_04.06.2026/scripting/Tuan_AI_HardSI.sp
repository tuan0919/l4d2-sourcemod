...
{
	name = "[L4D1/L4D2] Tuan AI: Hard SI (Elite Only)",
	author = "Breezy & HarryPotter, Tuan edit",
	description = "Hard SI behaviour only for Elite infected",
...
{
	if(!g_bCvarEnable) return Plugin_Continue;
	if(!ShouldApplyHardMovement(tank)) return Plugin_Continue;
...
{
	if(!g_bCvarEnable) return;
	if(!ShouldApplyHardMovement(tank)) return;
...
bool ShouldApplyHardMovement(int client)
{
	...
	// Keep backward compatibility when the elite-reward plugin is not loaded.
	if (!g_bL4D2Version || !g_bEliteNativeAvailable) {
		return true;
	}
