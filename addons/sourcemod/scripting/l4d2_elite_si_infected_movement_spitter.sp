...
{
	name = "[L4D2] Elite SI Movement Spitter",
	author = "OpenCode",
	description = "Movement subtype module for elite Spitter bots.",
...
{
	g_cvEnable = CreateConVar("l4d2_elite_si_infected_movement_spitter_enable", "1", "0=Off, 1=On.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_cvDelay = CreateConVar("l4d2_elite_si_infected_movement_spitter_delay", "0.0", "Delay before spitter movement unlock.", FCVAR_NOTIFY, true, 0.0, true, 5.0);
	g_cvSpeed = CreateConVar("l4d2_elite_si_infected_movement_spitter_speed", "250", "Spitter speed during movement window.", FCVAR_NOTIFY, true, 10.0, true, 1000.0);

	CreateConVar("l4d2_elite_si_infected_movement_spitter_version", PLUGIN_VERSION, "Plugin version.", FCVAR_NOTIFY | FCVAR_DONTRECORD);
	AutoExecConfig(true, "l4d2_elite_si_infected_movement_spitter");

	g_cvDefaultSpeed = FindConVar("z_spitter_speed");
