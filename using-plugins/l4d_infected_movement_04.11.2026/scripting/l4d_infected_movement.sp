...
bool g_bCvarAllow, g_bMapStarted, g_bLeft4Dead2, g_bEliteNativeAvailable, g_bEliteSubtypeNativeAvailable;
...
void Event_Use(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	if( !client || !IsClientInGame(client) ) return;
	if( !ShouldApplyAbilityMovement(client) ) return;
...
bool ShouldApplyAbilityMovement(int client)
{
	...
	return L4D2_GetEliteSubtype(client) == ELITE_SUBTYPE_ABILITY_MOVEMENT;
}
