...
{
	name = "[L4D2] Elite SI Abnormal Smoker",
	author = "OpenCode",
	description = "Abnormal Behavior module for elite Smoker.",
...
{
	g_cvEnable = CreateConVar("l4d2_elite_si_hardsi_smoker_enable", "1", "0=Off, 1=On.", FCVAR_NOTIFY, true, 0.0, true, 1.0);

	CreateConVar("l4d2_elite_si_hardsi_smoker_version", PLUGIN_VERSION, "Plugin version.", FCVAR_NOTIFY | FCVAR_DONTRECORD);
	AutoExecConfig(true, "l4d2_elite_si_hardsi_smoker");
...
public void OnActionCreated(BehaviorAction action, int actor, const char[] name)
{
	...
	if (!ShouldApplyAbnormalBehavior(actor, false, ZC_SMOKER))
	{
		return;
	}
