...
// Optional elite native to gate ChargerAction subtype
native bool L4D2_IsEliteSI(int client);
native int L4D2_GetEliteSubtype(int client);

#define ELITE_SUBTYPE_CHARGER_ACTION 3
...
ConVar g_hCvarEliteOnly, g_hCvarEliteSubtype;
...
bool g_bCvarEliteOnly;
int g_iCvarEliteSubtype;
bool g_bCvarAllow;
...
public void OnPluginStart()
{
	...
	g_hCvarEliteOnly = CreateConVar(	"l4d2_charger_elite_only", 	"1", 	"0=Apply ChargerAction to all chargers. 1=Only apply to chargers that are Elite ChargerAction subtype.", CVAR_FLAGS);
	g_hCvarEliteSubtype = CreateConVar("l4d2_charger_elite_subtype", "3", 	"Subtype id for ChargerAction elite (must match elite reward plugin).", CVAR_FLAGS);
	...
	g_hCvarEliteOnly.AddChangeHook(ConVarChanged_Cvars);
	g_hCvarEliteSubtype.AddChangeHook(ConVarChanged_Cvars);
...
void GetCvars()
{
	...
	g_bCvarEliteOnly = g_hCvarEliteOnly.BoolValue;
	g_iCvarEliteSubtype = g_hCvarEliteSubtype.IntValue;
...
{
	g_bEliteNativeAvailable = (GetFeatureStatus(FeatureType_Native, "L4D2_IsEliteSI") == FeatureStatus_Available) && (GetFeatureStatus(FeatureType_Native, "L4D2_GetEliteSubtype") == FeatureStatus_Available);
}

bool IsChargerActionEnabled(int client)
{
	...
	return L4D2_GetEliteSubtype(client) == g_iCvarEliteSubtype;
}
...
MRESReturn HandleCustomCollision(Handle hReturn, Handle hParams)
{
	...
	if( victim > 0 && victim <= MaxClients )
	{
		if( GetGameTime() > g_fThrown[victim] )
		{
			...
			{
				if( g_bCharging[i] && IsClientInGame(i) && GetClientTeam(i) == 3 && IsPlayerAlive(i) && IsChargerActionEnabled(i) )
				{
...
void Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	if( g_iCvarPickup )
	{
		if( event.GetInt("type") & DMG_CLUB )
		{
			...
			if( !client || IsFakeClient(client) || !IsChargerActionEnabled(client) ) return;
...
{
	if( client && IsClientInGame(client) && IsChargerActionEnabled(client) )
	{
...
void Event_PlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
	if( g_iCvarShove )
	{
		...
		if( IsChargerActionEnabled(target) && IsSurvivor(client) )
		{
...
void Event_PlayerIncap(Event event, const char[] name, bool dontBroadcast)
{
	if( g_iCvarFinish & (1<<1) )
	{
		...
		if( client && target && client <= MaxClients && target <= MaxClients && IsChargerActionEnabled(client) && IsClientInGame(target) && IsSurvivor(target) )
		{
			#if DEBUG
...
void Event_ChargeStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( !IsChargerActionEnabled(client) )
	{
		g_bCharging[client] = false;
		return;
	}
...
{
	client = GetClientOfUserId(client);
	if( client && g_bCharging[client] && IsClientInGame(client) && GetEntPropEnt(client, Prop_Send, "m_carryVictim") == -1 && IsChargerActionEnabled(client) )
	{
		float vLoc[3], vPos[3];
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3])
{
	...
	if( IsClientInGame(client) && !IsFakeClient(client) && IsPlayerAlive(client) && IsChargerActionEnabled(client) )
	{
		// JUMP
