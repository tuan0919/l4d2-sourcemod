...
public void Tuan_OnClient_HealedOther(int client, int victim)
{
    ...
    FormatChatNameFromClient(victim, victimFmt, sizeof(victimFmt));

    if (client == victim)
    {
        BuildChatLineActionOnly(clientFmt, "healed and no longer at last life", line, sizeof(line));
    }
    else
    {
        BuildChatLineActionWithTarget(clientFmt, "healed", victimFmt, line, sizeof(line));
    }
...
public void Tuan_OnClient_GoBnW(int client)
{
    ...
    char line[128];
    FormatChatNameFromClient(client, clientFmt, sizeof(clientFmt));
    BuildChatLineActionOnly(clientFmt, "is at last life", line, sizeof(line));
    PublishInfo(line);
}
...
public void Tuan_OnClient_RevivedOther(int client, int target)
{
    ...
    BuildChatLineActionWithTarget(clientFmt, "helped up", targetFmt, line, sizeof(line));
    PublishInfo(line);
}
...
public void Tuan_OnClient_UsedThrowable(int client, int throwable_type)
{
    ...
    switch (throwable_type)
    {
        case 0:
        {
            ...
            BuildChatLineActionOnly(clientFmt, "threw molotov", line, sizeof(line));
        }
        case 1:
        {
            if (!g_bNotifyThrowPipebomb) return;
            BuildChatLineActionOnly(clientFmt, "threw pipebomb", line, sizeof(line));
        }
        case 2:
        {
            if (!g_bNotifyThrowVomitjar) return;
            BuildChatLineActionOnly(clientFmt, "threw vomitjar", line, sizeof(line));
        }
        default:
        {
            return;
...
public void Tuan_OnClient_ExplodeObject(int client, int object_type)
{
    ...
    switch (object_type)
    {
        case TYPE_GASCAN:
        {
            ...
            BuildChatLineActionOnly(clientFmt, "exploded a gascan", line, sizeof(line));
        }
        case TYPE_FUEL_BARREL:
        {
            if (!g_bNotifyExplodeFuelBarrel) return;
            BuildChatLineActionOnly(clientFmt, "exploded a fuel barrel", line, sizeof(line));
        }
        case TYPE_PROPANECANISTER:
        {
            if (!g_bNotifyExplodePropane) return;
            BuildChatLineActionOnly(clientFmt, "exploded a propane canister", line, sizeof(line));
        }
        case TYPE_OXYGENTANK:
        {
            if (!g_bNotifyExplodeOxygen) return;
            BuildChatLineActionOnly(clientFmt, "exploded an oxygen tank", line, sizeof(line));
        }
        case TYPE_BARRICADE_GASCAN:
        {
            if (!g_bNotifyExplodeBarricade) return;
            BuildChatLineActionOnly(clientFmt, "exploded a barricade gascan", line, sizeof(line));
        }
        case TYPE_GAS_PUMP:
        {
            if (!g_bNotifyExplodeGasPump) return;
            BuildChatLineActionOnly(clientFmt, "exploded a gas pump", line, sizeof(line));
        }
        case TYPE_FIREWORKS_CRATE:
        {
            if (!g_bNotifyExplodeFireworks) return;
            BuildChatLineActionOnly(clientFmt, "exploded a fireworks crate", line, sizeof(line));
        }
        case TYPE_OIL_DRUM_EXPLOSIVE:
        {
            if (!g_bNotifyExplodeOilDrum) return;
            BuildChatLineActionOnly(clientFmt, "exploded an oil drum", line, sizeof(line));
        }
        default:
        {
            return;
