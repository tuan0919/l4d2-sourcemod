# Tuan_l4d2_death_incap_red (modular 05.07.2026)
...
## Thay doi 05/07/2026 - Explosive Chains compatibility

### Van de

- Khi nhieu survivor nem molotov/gascan gan nhau, resolver cu van co the nham source vi phai fallback theo position/time ring buffer.
- `l4d_explosive_chains` co the gan owner credit len fire/explosive chain bang `m_hOwnerEntity`, `m_hBreaker`, `m_hPhysicsAttacker`, nhung Red Announce truoc do chua uu tien cac prop nay khi chon source.

### Fix

1. Them owner hint resolver doc cac linked props:
   - `m_hOwnerEntity`
   - `m_hBreaker`
   - `m_hPhysicsAttacker`
   - `m_hThrower`
...
2. `ResolveDamageOwnerClient` va hazard damage tracking gio uu tien owner hint tu attacker/inflictor/hazard entity truoc khi fallback heuristic.

3. Fire source heuristic khi co owner hint se uu tien ring buffer da loc theo owner:
   - `FindRecentMolotovDestroyForOwner`
   - `FindRecentGascanExplodeForOwner`
   - `FindBestFireSourceForOwner`

4. `OnEntityDestroyed` cua hazard/molotov ghi ring buffer voi owner hint neu owner tracking noi bo dang rong.

5. `IsLikelyGascanInferno` va `HasExplicitMolotovEvidence` dung owner-filtered lookup khi co owner ro rang, tranh bi molotov/gascan cua survivor khac o gan do lam sai cause.

### Ghi chu

- Khong hard-require `l4d_explosive_chains`; neu plugin do khong load, Red Announce van fallback ve flow cu.
- Khi `l4d_explosive_chains` load, owner hint giup giam nham lan giua nhieu molotov/gascan cung luc o cung khu vuc.
