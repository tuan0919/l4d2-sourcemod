...
### Owner hint từ l4d_explosive_chains

Neu `l4d_explosive_chains` duoc load, plugin do co the gan survivor owner vao fire/explosive chain qua:
...
Red Announce 05.07.2026 doc cac prop nay bang `ResolveOwnerHintFromEntity` va dung nhu deterministic owner hint:

```
inferno/entityflame/hazard entity
        │
        ├─ ResolveOwnerHintFromEntity(entity)
        │       └─ neu tim thay survivor owner
        │
...
Muc tieu la giam nham lan khi nhieu survivor nem molotov/gascan gan nhau. Neu khong co owner hint, flow fallback theo position/time cu van duoc giu nguyen.
